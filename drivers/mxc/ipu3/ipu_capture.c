...
void ipu_cis_reg_dump(struct ipu_soc *ipu,  uint32_t csi)
{
	dev_dbg(ipu->dev, "csi = %d, CSI_SENS_CONF = 0x%08X\n",csi , ipu_csi_read(ipu, csi, CSI_SENS_CONF));
	...
	dev_dbg(ipu->dev, "CSI_TST_CTRL      = 0x%08X\n", ipu_csi_read(ipu, csi, CSI_TST_CTRL));
	dev_dbg(ipu->dev, "CSI_CCIR_CODE_1   = 0x%08X\n",ipu_csi_read(ipu, csi, CSI_CCIR_CODE_1));
	dev_dbg(ipu->dev, "CSI_CCIR_CODE_2   = 0x%08X\n", ipu_csi_read(ipu, csi, CSI_CCIR_CODE_2));
	dev_dbg(ipu->dev, "CSI_CCIR_CODE_3   = 0x%08X\n",ipu_csi_read(ipu, csi, CSI_CCIR_CODE_3));
	
	dev_dbg(ipu->dev, "CSI_MIPI_DI       = 0x%08X\n", ipu_csi_read(ipu, csi, CSI_MIPI_DI));
	dev_dbg(ipu->dev, "CSI_SKIP          = 0x%08X\n",ipu_csi_read(ipu, csi, CSI_SKIP));
	dev_dbg(ipu->dev, "CSI_CPD_CTRL      = 0x%08X\n", ipu_csi_read(ipu, csi, CSI_CPD_CTRL));

	dev_dbg(ipu->dev, "CSI_CPD_RC_0      = 0x%08X\n",ipu_csi_read(ipu, csi, CSI_CPD_RC(0)));
	dev_dbg(ipu->dev, "CSI_CPD_RS_0      = 0x%08X\n", ipu_csi_read(ipu, csi, CSI_CPD_RS(0)));
	dev_dbg(ipu->dev, "CSI_CPD_GRC_0     = 0x%08X\n",ipu_csi_read(ipu, csi, CSI_CPD_GRC(0)));
	dev_dbg(ipu->dev, "CSI_CPD_GRS_0     = 0x%08X\n", ipu_csi_read(ipu, csi, CSI_CPD_GRS(0)));
	dev_dbg(ipu->dev, "CSI_CPD_GBC_0     = 0x%08X\n",ipu_csi_read(ipu, csi, CSI_CPD_GBC(0)));
	dev_dbg(ipu->dev, "CSI_CPD_BC_0      = 0x%08X\n", ipu_csi_read(ipu, csi, CSI_CPD_BC(0)));
	dev_dbg(ipu->dev, "CSI_CPD_BS_0      = 0x%08X\n",ipu_csi_read(ipu, csi, CSI_CPD_BS(0)));

	dev_dbg(ipu->dev, "CSI_CPD_OFFSET1   = 0x%08X\n", ipu_csi_read(ipu, csi, CSI_CPD_OFFSET1));
	dev_dbg(ipu->dev, "CSI_CPD_OFFSET2   = 0x%08X\n",ipu_csi_read(ipu, csi, CSI_CPD_OFFSET2));
...
//	_ipu_csi_set_test_generator(ipu, 1, 0, 0xff , 0,  27000000, csi);   // the csi1 test mode.
//	ipu_csi_write(ipu, csi, 0x04008818, CSI_SENS_CONF);                 // the csi1 test mode. the chess board come out.
//  ipu_cis_reg_dump(ipu, csi); 	
	mutex_unlock(&ipu->mutex_lock);
