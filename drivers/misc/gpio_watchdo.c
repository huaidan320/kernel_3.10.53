...
#define DEVICE_NAME	"gpio_watchdog"
#define STATE_HIGH               1
#define STATE_LOW                0
...
{
	if (!watchdog_awlays_en) {
		watchdog_disable();
		///printk("watchdog: disable watchdog\n");
	}
	else {
		printk("Watchdog has always been open!\n");
	}
	return 0;
}
...
static int watchdog_probe(struct platform_device *pdev)
{
	...
	watchdog->feeddog_pin = of_get_gpio(pdev->dev.of_node, 0);
	...
	if (!of_find_property(pdev->dev.of_node, "no_enable", NULL)) {
		watchdog->enable_pin = of_get_gpio(pdev->dev.of_node, 1);
	...
	if(ret < 0) {
		printk("gpio watchdog: misc register error\n");
		goto exit_kfree;
	}
	else {
		printk("gpio watchdog: misc register successed\n");
	}
...
static const struct of_device_id atmel_wdt_dt_ids[] = {
	{ .compatible = "gpio_watchdog"},
	{ /* sentinel */ }
}

MODULE_DEVICE_TABLE(of, atmel_wdt_dt_ids);
#endif

static struct platform_driver sama5d3_wdt_driver = {
	.probe		= watchdog_probe,
	.remove		= watchdog_remove,
	.driver		= {
		.name	= "gpio_watchdog",
		.of_match_table = of_match_ptr(atmel_wdt_dt_ids),
	},
};

static int __init watchdog_init(void) 
{
	return platform_driver_register(&sama5d3_wdt_driver);
}

static void __exit watchdog_exit(void)
{
	platform_driver_unregister(&sama5d3_wdt_driver);
}

module_init(watchdog_init); 
module_exit(watchdog_exit);

MODULE_DESCRIPTION("Driver for GPIO Watchdog");
MODULE_AUTHOR("pxh");
MODULE_LICENSE("GPL");
