...
//		pr_info("----- status=%x, buf_tail=%d of %d bd[0].mode.status=%x\n", bd->mode.status, sdmac->buf_tail, sdmac->num_bd, sdmac->bd[0].mode.status);
//		pr_info(":::::%s %d tail=%d num_bd=%d current_bd_ptr=%x count=%d status=%x\n",
//							__func__, __LINE__, sdmac->buf_tail,
//							sdmac->num_bd, sdmac->sdma->channel_control[sdmac->channel].current_bd_ptr,
//							bd->mode.count, bd->mode.status);
		...
		if (bd->mode.status & BD_RROR) {
			...
			pr_info("!!!!! DMA_ERROR status=%x, buf_tail=%d of %d\n", bd->mode.status, sdmac->buf_tail, sdmac->num_bd);
		}
//		for (i = 0; i< sdmac->num_bd; i++) {
//			pos += sprintf(info + pos, "[%d]%x; ", i, sdmac->bd[i].mode.status);
//			sdmac->bd[i].mode.status |= BD_DONE;
//		}
//		pr_info("----->> tail=%d %s\n", sdmac->buf_tail, info);

//		pr_info("%d ===== bd->mode.status=%x tail=%d\n", __LINE__, bd->mode.status, sdmac->buf_tail);
		bd->mode.status |= BD_DONE;
//		pr_info("%d ===== bd->mode.status=%x tail=%d\n", __LINE__, bd->mode.status, sdmac->buf_tail);
		sdmac->buf_tail++;
		sdmac->buf_tail %= sdmac->num_bd;
		if (0 == sdmac->buf_tail) {
			pr_info("reset to bd0, num_bd=%d\n", sdmac->num_bd);
		}
...
//		for (i = 0; i< sdmac->num_bd; i++)
//			pos += sprintf(info + pos, "[%d]0x%x; ", i, sdmac->bd[i].mode.status);
//		pr_info("%d ===== bd->mode.status=%x tail=%d\n", __LINE__, bd->mode.status, sdmac->buf_tail);
//		pr_info("<<----- tail=%d %s\n", sdmac->buf_tail, info);
//		bd->mode.status |= BD_DONE;
	}
...
static irqreturn_t sdma_int_handler(int irq, void *dev_id)
{
	...
	while (stat) {
		...
		if (sdmac->status == DMA_IN_PROGRESS || sdmac->mode == SDMA_MODE_LOOP) {
//			pr_info(":::::%s %d mode=%d\n", __func__, __LINE__, sdmac->mode);
//			if (sdmac->mode == SDMA_MODE_LOOP && (1 == (sdmac->num_bd - sdmac->buf_tail))) {
//				char info[200];
//				int i, pos=0;
...
//					sdmac->bd[i].mode.status |= BD_DONE;
//				}
//				pr_info(":::::%s %d ch disabled mode=%d tail=%d num_bd=%d current_bd_ptr=%x\n",
//						__func__, __LINE__, sdmac->mode, sdmac->buf_tail, sdmac->num_bd, sdma->channel_control[channel].current_bd_ptr);
//				for (i = 0; i< sdmac->num_bd; i++) {
//					pos += sprintf(info + pos, "[%d]%x; ", i, sdmac->bd[i].mode.status);
//					sdmac->bd[i].mode.status |= BD_DONE;
//				}
//				pr_info("----->> tail=%d %s\n", sdmac->buf_tail, info);
//				sdma_issue_pending(&sdmac->chan);
//				for (i = 0; i< sdmac->num_bd; i++) {
//					pos += sprintf(info + pos, "[%d]%x; ", i, sdmac->bd[i].mode.status);
//					sdmac->bd[i].mode.status |= BD_DONE;
//				}
//				pr_info("<<----- tail=%d %s current_bd_ptr=%x\n", sdmac->buf_tail, info, sdma->channel_control[channel].current_bd_ptr);
//			}
...
static int sdma_request_channel(struct sdma_channel *sdmac)
{
	...
	pr_info("%s sdma iram=%s channel num=%d, type=%d\n", __func__,
			sdmac->bd_iram ? "true":"false", sdmac->channel, sdmac->peripheral_type);
...
static int __init sdma_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "iram_pool=%p\n", sdma->iram_pool);
	if (!sdma->iram_pool)
		dev_err(&pdev->dev, "no iram assigned, using external mem\n");
