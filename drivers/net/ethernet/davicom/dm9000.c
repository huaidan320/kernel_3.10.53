...
dm9000_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* free this SKB */
	dev_kfree_skb(skb);				/// pxh@hzmct.com 2015.1.22
...
static struct dm9000_plat_data *dm9000_parse_dt(struct device *dev)
{
	...
	if (of_find_property(np, "davicom,16bit-data", NULL))
...
static void
dm9000_test(struct board_info *db)
{
	u32 i;
	u32 id_val;
...
#if 1
	writew(0x00, db->io_data);
	udelay(200);
	writew(0xffff, db->io_data);
	udelay(200);
#else
	...
	for (i = 0; i < 8; i++) {
		printk("read id 0x%x 0x%x 0x%x 0x%x\n", ior(db, DM9000_VIDL),ior(db, DM9000_VIDH),
			ior(db, DM9000_PIDL),(u32)ior(db, DM9000_PIDH));
	}
#endif
}
...
///	printk("start:0x%x end:0x%x\n",db->addr_res->start, db->addr_res->end);
///	printk("start:0x%x end:0x%x\n",db->data_res->start, db->data_res->end);
///	printk("start:0x%x end:0x%x\n",db->irq_res->start, db->irq_res->start);
