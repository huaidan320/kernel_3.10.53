...
SOC_DOUBLE_R("Capture Switch", WM8960_LINVOL, WM8960_RINVOL,
	7, 1, 1),	/// pxh 2015.2.10 fix bit xinvert
...
static const struct snd_soc_dapm_route audio_paths[] = {
	{ "LINPUT1", NULL, "MICB"},			/// pxh add 2015.2.10
...
static int wm8960_probe(struct snd_soc_codec *codec)
{
	...
	/// The ADCLRC/GPIO1 pin function should not be modified while the ADC is enabled
	if (pdata->shared_lrclk) {
		reg = snd_soc_read(codec, WM8960_POWER1);
		snd_soc_write(codec, WM8960_POWER1, reg & 0x1f2);
	
		snd_soc_write(codec, WM8960_ADDCTL2, 0x4);
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (!pdata) {
		wm8960->pdata = devm_kzalloc(&i2c->dev, sizeof(struct wm8960_data),
			      GFP_KERNEL);
		if (wm8960->pdata == NULL) {
			devm_kfree(&i2c->dev, wm8960);
			return -ENOMEM;
		}
		...
		if (i2c->dev.of_node) {
			...
			if (ret != 0) {
				devm_kfree(&i2c->dev, wm8960->pdata);
				devm_kfree(&i2c->dev, wm8960);
				return ret;
			}
		}
	...
	if (pdata && pdata->shared_lrclk) {
		ret = regmap_update_bits(wm8960->regmap, WM8960_ADDCTL2,
					 0x4, 0x4);
		if (!ret)
			ret = regmap_update_bits(wm8960->regmap, WM8960_IFACE2,
					 0x40, 0x40);
		if (ret != 0) {
